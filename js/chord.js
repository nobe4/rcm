...
const modulationLists = {
	...
	'full'   : ['', 'maj', 'min', 'dim', 'aug', '5', '7', 'maj7', 'min7', 'minmaj7', 'dim7', 'hdim7', 'dom7', 'aug7']
};
...
function modulationToNoteNames(root, modulation){
	...
	if (!chordModulations.hasOwnProperty(modulation)) throw `Invalid chord name: '${modulation}'.`;
...
function chordsToHTML(chords){
	return chords.map(chord => {
			let note = mtnn(chord.root);
			return `<li class="chord">
				<span class="rhythm">
					${chord.durationString.split('').map(a=>`<i class='${a}'></i>`).join('')}
				</span>
				<span class="note">
				...
				<ul class="details">
				${modulationToNoteNames(chord.root, chord.modulation).map(note => {
...
class Chord {
	...
	static random(duration=undefined, modulationListName='simple'){
		...
		let root = 57 + (Math.random()*12 | 0);

		let modulationList = modulationLists[modulationListName];
		let modulation =  modulationList[Math.floor(Math.random()*modulationList.length)];
