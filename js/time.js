...
function stringToRatio(durationString){
	...
	let durationValues = durationString.split('');
	let ratio = 0;
...
class RhythmBox {
	...
	/* simpleGenerator: Create the default duration for the defined duration, simply cut the bar in
	...
	equalGenerator(){
		...
		let duration = Math.ceil(this.duration/4.0);
		let durations = [duration, duration, duration, duration];
		// Compute the difference between the computed duration and the expected
		// one.
		let difference = duration * 4 - this.duration;
	...
	randomGenerator(noise=20){
		...
		let divisionBaseSize = Math.ceil(this.duration/(Math.random()*this.duration));

		// Max number of divisions
		let maxDivisions = Math.floor(this.duration/divisionBaseSize);

		// How many divisions we want.
		let divisionCount = Math.floor(Math.random()*maxDivisions) + 1;

		// Actual division size
		let divisionSize = Math.ceil(this.duration / divisionCount);

		// Array of durations
		let durations = Array.from({length: divisionCount}, ()=>divisionSize);

		// How much time is added in the computation.
		let difference = divisionCount * divisionSize - this.duration;
		...
		if(divisionCount > 1){
			for(let x = Math.random()*noise; x --> 0;){
				let a = Math.floor(Math.random()*divisionCount);
				let b = Math.floor(Math.random()*divisionCount);
	...
	binaryGenerator(){
		...
		let divisions = 2 + Math.floor(Math.random()*(this.duration/4));

		for(let x = divisions; x --> 0;){
			let randomIndex = Math.floor(Math.random()*durations.length);

			let divisibleValue = durations[randomIndex];
